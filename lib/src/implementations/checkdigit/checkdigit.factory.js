...
const arbiter_scopes_1 = require("./arbiter.scopes");
...
class CheckdigitOidFactory {
    static GetHashidOidOptions(scopename) {
        switch (scopename) {
            case arbiter_scopes_1.ArbiterScopeNames.Workflow:
            case arbiter_scopes_1.ArbiterScopeNames.Activity:
            case arbiter_scopes_1.ArbiterScopeNames.BusinessApplication:
            case arbiter_scopes_1.ArbiterScopeNames.ExternalEvent:
                return {
                    length: 5,
                    checksum: 3,
                    salt: 'rfi_oid'
    ...
    create(scopename, id) {
        ...
        return new oid_1.Oid2(`${shortcode}_${suffix}${check_digit}`);
    }
    verifyAndStripCheckDigit(scope, shortcode, suffix) {
        const { checksum, length } = CheckdigitOidFactory.GetHashidOidOptions(scope);
        if (Reflect.get(arbiter_scopes_1.ArbiterScopeNames, scope) && suffix.length === length) {
