...
class ScopeRegistry {
    static getScopeType(scopename) {
        if (Reflect.get(plain_1.OidScopeNames, scopename)) {
            return types_1.ScopeTypes.OID;
        }
        if (Reflect.get(checkdigit_1.HashidScopeNames, scopename)) {
            return types_1.ScopeTypes.HASHID;
        }
    ...
    register(scopename, shortcode) {
        ...
        switch (ScopeRegistry.getScopeType(scopename)) {
            ...
            case types_1.ScopeTypes.OID:
                ...
                return shortcode;
            case types_1.ScopeTypes.HASHID:
                if (!shortcode) {
