...
const scope_to_factory_lookup_1 = require("./lookup/scope-to-factory.lookup");
...
class Oid2 {
    ...
    static getFactoryByScopename(scopename) {
        const factory = scope_to_factory_lookup_1.OidFactoryMapByScope.get(scopename) || new factories_1.ModernOidFactory();
        if (!factory) {
    ...
    static getFactoryByEncoded(external_oid) {
        if (external_oid[0] === `~`) {
            return new factories_1.TildeOidFactory();
        }
