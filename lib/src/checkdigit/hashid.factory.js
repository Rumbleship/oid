...
const checkdigit_1 = require("../implementations/checkdigit");
class HashidOidFactory {
    static GetHashidOidOptions(scopename) {
        switch (scopename) {
            case checkdigit_1.HashidScopeNames.Buyer:
                return {
                    shortcode: 'b',
                    checksum: 2,
                    salt: 'Division'
                };
            case checkdigit_1.HashidScopeNames.Supplier:
                return {
                    shortcode: 's',
                    checksum: 2,
                    salt: 'Division'
                };
            case checkdigit_1.HashidScopeNames.User:
                return {
                    shortcode: 'u',
                    checksum: 2,
                    salt: 'User'
                };
            case checkdigit_1.HashidScopeNames.PurchaseOrder:
                return {
                    shortcode: 'po',
                    checksum: 3,
                    salt: 'purchaseOrder'
                };
            case checkdigit_1.HashidScopeNames.Shipment:
                return {
                    shortcode: 'shp',
                    checksum: 5,
                    salt: 'shipment'
